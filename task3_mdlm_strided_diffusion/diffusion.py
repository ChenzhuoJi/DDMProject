...
class Diffusion(L.LightningModule):
  ...
  def _build_timesteps(self, sampling_steps, eps, schedule):
    """Build a descending timestep sequence with per-step dt values.

    Args:
      sampling_steps: number of denoising steps to actually execute.
      eps: lower bound of t (avoids log(0) in noise schedules).
      schedule: 'linear' or 'quadratic'.
        linear    – uniform spacing, identical to the original behaviour.
        quadratic – steps are denser near t=0 so the model takes smaller
                    jumps in the low-noise region where errors matter most.
                    Concretely we map u ~ uniform[0,1] through u^2 so that
                    t_i = (1 - eps) * (1 - (i / N)^2) + eps, giving more
                    steps where the signal is cleanest.
    Returns:
      timesteps: 1-D tensor of length `sampling_steps`, descending,
                 t[0] ≈ 1, t[-1] ≈ eps.
      dts:       1-D tensor of length `sampling_steps`, dt[i] = t[i] - t[i+1],
                 with a sentinel t[N] = eps appended before differencing.
    """
    if schedule == 'quadratic':
      # u_i = i/N, mapped through 1 - u^2  so early steps are large,
      # late steps (near t=0) are small.
      i = torch.arange(sampling_steps + 1,
                       dtype=torch.float64, device=self.device)
      u = i / sampling_steps                      # [0, 1/N, 2/N, ..., 1]
      t_all = (1 - eps) * (1 - u ** 2) + eps     # descending, [1, ..., eps]
    else:
      t_all = torch.linspace(1, eps, sampling_steps + 1,
                             dtype=torch.float64, device=self.device)

    timesteps = t_all[:-1].float()   # shape (N,)  — the query points
    dts = (t_all[:-1] - t_all[1:]).float()  # shape (N,)  — per-step jumps, all > 0
  ...
  def _sample(self, num_steps=None, eps=1e-5):
    ...
    # ── strided time schedule ──────────────────────────────────────
    sampling_steps = getattr(
      self.config.sampling, 'sampling_steps', num_steps)
    time_schedule = getattr(
      self.config.sampling, 'time_schedule', 'linear')
    timesteps, dts = self._build_timesteps(sampling_steps, eps, time_schedule)
    # ───────────────────────────────────────────────────────────────
    ...
    for i in range(sampling_steps):
      ...
      dt = dts[i].item()               # scalar float for this stride
